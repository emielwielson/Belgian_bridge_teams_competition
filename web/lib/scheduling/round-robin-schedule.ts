/**
 * Generic circle-method round robin for regional groups (any team count).
 */
...
export function computeRoundCount(
  ...
  return roundsPerCycle(teamCount) * Math.max(roundRobinCount, 1);
}

/** One full round-robin cycle using the circle method. */
...
export function buildRoundRobinSchedule(
  ...
  for (let cycle = 0; cycle < cycles; cycle++) {
    const leg = cycle % 2 === 0 ? cyclePlans : cyclePlans.map((plan) => ({
      round: plan.round,
