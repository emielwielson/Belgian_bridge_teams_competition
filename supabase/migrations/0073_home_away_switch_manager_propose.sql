-- Allow competition managers who manage a match to propose home/away switches
-- (same pattern as postponement manager propose).
...
begin
  ...
  if not (
    public.current_user_is_captain_of_team(p_requesting_team_id)
    or public.current_user_manages_match(p_match_id)
  ) then
    raise exception 'Only the team captain or a competition manager may propose a home/away switch';
  end if;
